...
void HDF5Writer::write_run_provenance(const Simulation& sim) {
  ...
  const auto drift_correction =
      static_cast<int32_t>(sim.config().qssa.drift_correction);
  write_scalar_dataset(fid, "run_provenance/neumann_drift_correction",
                       H5T_NATIVE_INT32, &drift_correction);
...
void HDF5Writer::write_run_termination(const Simulation& sim, Int step,
  ...
  if (enabled_ && io_rank(cfg_) == 0 && file_id_ >= 0) {
    ...
    const auto drift_correction =
        static_cast<int32_t>(sim.config().qssa.drift_correction);
    write_scalar_dataset(
        fid, "run_provenance/neumann_drift_correction",
        H5T_NATIVE_INT32, &drift_correction);
    const auto negative_count = static_cast<unsigned long long>(
        sim.neumann_negative_field_count());
    write_scalar_dataset(fid, "run_provenance/neumann_negative_field_count",
...
void HDF5Writer::write_summary(Simulation& sim, const std::string& group,
  ...
  write_flux("maintenance_shortfall_interval",
             flux.maintenance_shortfall_interval);
  write_flux("maintenance_shortfall_cumulative",
             add_cumulative(flux.maintenance_shortfall_cumulative,
                           flux.maintenance_shortfall_interval));
  write_flux("maintenance_limited_agents_interval",
             flux.maintenance_limited_agents_interval);
  write_flux("maintenance_limited_agents_cumulative",
             add_cumulative(flux.maintenance_limited_agents_cumulative,
                           flux.maintenance_limited_agents_interval));
  write_flux("uptake_demand_interval", flux.uptake_demand_interval);
