...
double mode_sum(double z_source, double z_target, double rho,
  ...
  const bool negative_lower = robin_boundary
      || sealed_reference == SealedReference::Physical;
  const double lower = negative_lower ? -a : a;
...
double normalized_image_series_impl(double z_source, double z_target, double rho,
  ...
  const double flow_magnitude = std::sqrt(
      flow_x * flow_x + flow_z * flow_z);
  const auto kernel = [z_target, rho, d_eff, decay_rate, flow_x, flow_z](
                          double image_z, int reflected) {
