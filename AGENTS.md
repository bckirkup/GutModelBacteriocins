...
| **Clamped delivery realized-sink ledger** | Fixed | Delivery realized-sink accounting now remains signed through replicated periodic x/y, slab x/y, bounded-z, and CUDA paths. Intra-step negative delivery excursion events are reported per species on host paths; CUDA carries the signed mass ledger but does not yet report the diagnostic. Final concentration clamping and `reaction_clip` remain separate, and no claim is made that intra-step positivity is enforced. |
