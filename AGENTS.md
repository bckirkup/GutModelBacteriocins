...
| **Wall-normal drift in the sealed image series** | Gated, not fixed | Reflected images reverse `U_z`, so each solves the mirrored operator (exact residual `-2 U_z ∂f/∂z`) and the pair enforces zero *diffusive* flux instead of the physical zero *total* flux, leaking `U_z C` at both walls. Measured relative field error is first order in `Pe_z`: median `0.114*Pe_z`, worst case `0.439*Pe_z`, exact at `Pe_z=0`. Drift-corrected images cannot repair it — the exact reflection coefficient `(γ-a)/(γ+a)` is transverse-wavenumber dependent and no per-image exponential weight is; all constant weights buy <=1.5x. Gated by `qssa.drift_envelope_policy` on `|Pe_z| <= 0.05` with `neumann_drift_envelope_evaluations` provenance; shipped spec-basis midpoint `Pe_z=0.164` and configured-plasmid ColE1 basis `Pe_z=0.822` at mid-depth and `2.325` at the lumen surface are outside the envelope, so the default warning fires. The audit's sub-percent characterization applies only to weakly retarded toxins; strongly retarded ColE1 and colicin M have 5-15% median and 20-40% worst-case field errors. Whether killing or induction outcomes move is untested because the corrected modal solve is not implemented and existing toxin/killing tolerances may encode the uncorrected field; this is a scientific decision for the maintainer. The series arithmetic is unchanged by this gate-only change. See `docs/NEUMANN_WALL_NORMAL_DRIFT.md`. |
