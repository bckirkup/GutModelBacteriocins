...
| **Carbon reference amplitude has no config key** | Open | `carbon.boundary_conc` re-pins only the epithelial cell while the prescribed z-gradient keeps `initial_conc = 5e-3` as its amplitude, so setting the boundary alone inverts the profile — measured at `carbon.boundary_conc=1e-3`: C(2 µm)=1.00e-3 below C(26 µm)=1.75e-3, carbon rising away from its source. The carbon capacity ladder needs the amplitude and the boundary to move together; see `docs/CARBON_LADDER_CAMPAIGN.md`. |
