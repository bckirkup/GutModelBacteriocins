...
| **Non-delivery GPU oxygen VBF discretization** | Open | Delivery-enabled oxygen uses the host's implicit first-order sink-rate diagonal, while ordinary non-delivery GPU oxygen VBF retains its shipped explicit reaction update. With the shipped `oxygen.vbf_sink = 1e-3 1/s` and `dt = 60 s`, one step gives explicit factor `1 - s·dt = 0.94` versus implicit factor `1/(1 + s·dt) = 0.9433962264`: removed fractions differ by 6.0% relative to implicit removal, and the resulting field differs by `0.0033962264·C` (0.36% relative to the implicit concentration). Scientific significance remains an open decision. |
