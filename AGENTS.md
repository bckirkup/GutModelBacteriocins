...
| GPU portability | Open, cost/benefit measured | Production chemistry + mechanics on GPU; CDI corpse participation is now parity-preserving through a shared host/device predicate; FMM M2L tree-walk on CPU; multi-GPU NCCL not wired. Host coverage is local; device coverage is evidenced only by the T4 CI job. Measured on a T4 at 500x500x50: device chemistry is 1.29x *slower* than host for `uptake_limit=none`/`sherwood` (transfer alone is 19.3 s of 49.9 s) and 1.97x faster for `delivery` via Route B; the 3.8x end-to-end gain at 1e5 agents is QSSA Green's-function sampling in biology, and it falls to 1.36x at 1e6 agents as that phase shrinks to a small share of biology. Device `spatial_hash_s`/`mpi_reaction_reduce_s` inflation and the host-only drift-envelope counter are unexplained measurement artifacts, not netted out. See `docs/GPU_COST_BENEFIT_2026-08.md`. |
