...
| **Wall-normal drift in the sealed image series** | Fixed, opt-in correction | Reflected images reverse `U_z`, so the legacy series and previous in-tree sealed modal field enforce zero diffusive flux instead of physical zero total flux (wall-flux residual `+0.451` for both versus `2e-10` for the corrected sign). The old `0.114/0.439·|Pe_z|` coefficients were calibrated against that wrong reference; the physical nine-probe envelope is median `~0.167·|Pe_z|` / max `~0.439·|Pe_z|`. The opt-in `qssa.drift_correction` path uses the physical modal field and a 33³ log-rho table against the exact runtime image series; the old Hankel cross-check was not independent because it encoded the same reflection coefficient. Defaults remain byte-identical; corrected GPU diffusion falls back to host. See `docs/NEUMANN_WALL_NORMAL_DRIFT.md`. |
...
| **Shipped ROS/uptake defaults after the delivery/ROS campaign** | Partially decided | `oxygen.k_ROS` now ships as `0.0`, so the ambient ROS mortality path is retired by default while remaining available through an explicit positive coefficient. `oxygen.ros_driver=ambient` and `metabolism.uptake_limit=none` remain shipped; explicit GPU delivery is now permitted with host-forced delivery chemistry and `/run_provenance/chemistry_placement` provenance. The remaining open decision is whether and how to enable funded uptake/ROS defaults after benchmark evidence; the device delivery port itself has landed for the narrow scope recorded in `docs/CUDA_DELIVERY_PARITY.md`. |
| **CUDA delivery parity** | Fixed, narrow device scope | Route B solves delivery diffusion on device for replicated single-rank runs with line lengths up to 512, carrying both sink terms (agent uptake as prescribed RHS mass, the first-order VBF sink in the implicit diagonal) with per-cell realized removal read back to fund biomass; the positivity rationing policy stays single-sourced on the host, driven by device-backed negativity and reduction callbacks. Everything outside that scope — slab mode, `nprocs > 1`, longer lines, unsupported boundary combinations, incomplete device capabilities — falls back to Route A and records `host_forced_delivery` in `/run_provenance/chemistry_placement`; `device_delivery` is recorded only when the device path ran. Device evidence is the T4 job only. Delivery-species eligibility is preflighted for the whole field before any device mutation, so a post-preflight CUDA failure throws instead of half-applying the step. |
| **Non-delivery oxygen VBF discretization** | Fixed | The explicit first-order VBF update was present identically on host and device; the issue was a delivery/non-delivery integration-scheme difference, not a GPU-specific divergence. Non-delivery oxygen is now implicitly integrated through the shared closed-form helper, matching delivery-mode treatment. |
| **Non-delivery iron VBF discretization** | Open | Iron VBF deliberately retains its explicit first-order form. At the shipped iron rate `1e-4 1/s` and `dt = 60 s`, `s·dt = 6e-3`, giving approximately `0.6%` explicit removal and a `0.006%` concentration difference versus implicit integration. Consistency is a pending maintainer decision. |
| **Post-step nutrient `*_step` reads** | Fixed | `boundary_step`, `gradient_source_step`, `vbf_source_step`, `vbf_sink_step`, `agent_uptake_step`, and `reaction_clip_step` are mid-step scratch vectors. The commit methods copy their values to `*_last_step` and then zero the scratch vectors, so post-step readers must use the corresponding `*_for_step()` accessors; reading the raw `*_step` members after `Simulation::step()` returns exactly zero. |
| **Delivery VBF sink ledger overlap** | Fixed | For delivery-enabled species, `vbf_sink_step`/`vbf_sink_for_step()` is the realized VBF share contained in `total_sink_realized`, so subtracting both double-counts removal. `finalize_delivery_realized()` combines the VBF and agent shares, while the `add_interval()` call in `chemistry_pipeline.cpp` reports the VBF share; use `total_sink_realized` alone in a delivery conservation sum and assert VBF-share containment separately. For non-delivery species, `vbf_sink` remains an independent removal channel. |
| **Stale device toxin field when sources vanish** | Fixed | Host-only zeroing plus `launch_superpose` returning false for empty sources leaves the device row un-memset; device receptor kill reads `conc_device`, and post-loop host sync resurrects the stale row. Device evidence is from the T4 job only. |
| **Concentration mirror residency** | Fixed | Per-species host-dirty tracking elides the whole-field `step_conc_upload` (`11.0 GB`, `18%` of measured A4/s1 traffic) and zeroed-row `qssa_toxin_zero` (`2.9 GB`, `5%`) transfers. Device evidence is from the T4 job only; every new host concentration writer must mark its species dirty. |
| **Reaction mirror residency** | Fixed | Per-species host-dirty tracking keeps device reaction shares resident and elides the whole-field `chem_reactions` round trip (`22.0 GB`, `37%` of measured A4/s1 traffic). Device evidence is from the T4 job only; every new host reaction writer must mark its species dirty. |
| **Clamped delivery realized-sink ledger** | Fixed | Delivery realized-sink accounting now remains signed through replicated periodic x/y, slab x/y, bounded-z, and CUDA paths. Intra-step negative delivery excursions are a directional-splitting artifact of the zero-order prescribed term, bounded at 1.33% of realized field-side removal (1.086% in the #390 fixture); end-of-step positivity and ledger closure to 1.75e-12 remain intact; per-axis diagnostics report them per run. Enforcement was rejected because a no-inflow per-axis cap defers 99.94% of prescribed mass and changes even the saturated regime. CUDA carries the signed mass ledger but does not yet report the host per-axis diagnostic. |
...
| GPU portability | Open, cost/benefit measured | Production chemistry + mechanics on GPU; CDI corpse participation is now parity-preserving through a shared host/device predicate; FMM M2L tree-walk on CPU; multi-GPU NCCL not wired. Host coverage is local; device coverage is evidenced only by the T4 CI job. Measured on a T4 at 500x500x50 after the placement fixes and with OpenMP on: device chemistry is 1.91x *faster* than host for `uptake_limit=none` (21.41 vs 11.18 s) and end-to-end is 2.86x at 1e5 agents, of which biology is 4.71x. The earlier campaign's 1.29x device chemistry *regression* was host<->device marshalling charged to the chemistry phase, not device arithmetic, and its 3.8x end-to-end figure was against a serial host: the benchmark image built without OpenMP (`openmp_compiled: 0` in every campaign pass) and the step-start whole-field mirror upload plus a single-rank reaction round trip inflated `spatial_hash_s`/`mpi_reaction_reduce_s`. Those copies are removed and the image now passes `-DGUTIBM_USE_OPENMP=ON`. Transfers are now measured with bytes and call counts: 57 GB per ten steps at 4.7-4.8 GB/s pageable, 9.1% of device wall, so residency, not pinning, is the remaining lever. Only the A1/A4 `s1` pair is post-fix; the `s2` ratios (1.36x at 1e6 agents) and the A2/A3/A5/A6 arms are pre-fix and pre-OpenMP. The host-only drift-envelope counter remains blind on device. See `docs/GPU_COST_BENEFIT_2026-08.md`. |
...
| **Cross-boundary HGT to ghost recipient** | Fixed | Not an under-count: with no ghost-recipient guard a boundary-straddling pair was attempted twice per step, once on each rank, and only the recipient-owner's genome write survived the ghost clear while both ranks incremented `conjugation_transfers` and appended a lineage HGT edge. A 4-rank fixture recorded 7 transfers and 7 edges for 4 acquisitions against 4/4/4 serially; acquisition probability was unbiased, so the corruption was in the ledger and genealogy, not the biology. Acquisitions now commit only on the recipient's owning rank; ghost donors carry the cross-boundary case, so each pair is attempted exactly once globally with no added communication. `domain.ghost_width` must stay >= pili reach or the recipient's owner never sees the donor — `FixConjugation::init` warns. Measured affected fraction and shipped-default reachability in `docs/EXTERNAL_AUDIT_2026-08.md`. |
...
| Image series with wall-normal flow | Open | Translations + z-reversed reflections satisfy zero diffusive flux at both walls but do not solve the drift PDE when U_z != 0. The enforced gate uses the configured-species basis (shipped midpoint Pe_z=0.164) and configured-plasmid basis (ColE1 Pe_z=0.822 at mid-depth, 2.325 at the lumen surface); the default warning fires because both exceed 0.05. The audit's sub-percent characterization applies only to weakly retarded toxins; strongly retarded ColE1 and colicin M have 5-15% median and 20-40% worst-case field errors. The arithmetic is unchanged, and killing/induction outcome movement remains untested. |
...
| **Dynamic mucin carbon liberation is dimensionally wrong** | Open | `dynamic_mucin_liberation()` returns `k_liberation [1/s] * vbf.density [cells/m^3] * Monod(M)`, i.e. `1e-4 * 1e11 = 1e7` mol/m^3/s at defaults — eleven orders above the static `vbf.mucin_liberation` term it replaces. Measured with only `mucin.enabled=true`: carbon reaches `3.4e7` mol/m^3 at 26 µm after one 60 s step against a `5e-3` physiological scale, then washes out over roughly ten steps. Mucin is also emptied outside the epithelial layer in one step while carbon receives the full unlimited amount, so the two species are not stoichiometrically coupled, and `mucin.secretion_rate` is inert because liberation is Monod-saturated (`M=1e-2` vs `Km_degradation=1e-3`). `mucin.enabled` is `false` by default, so no shipped result is affected; a mucin-driven carbon arm must not be run until `k_liberation` is redefined as a per-organism rate (mol/(cell·s)) with mucin-limited release. See `docs/CARBON_LADDER_CAMPAIGN.md`. |
| Image series with wall-normal flow | Open | translations + z-reversed reflections satisfy zero diffusive flux at both walls but do not solve the drift PDE when U_z != 0 (residual 2.9e-1 vs 1.4e-3 for a converged mode sum); sub-percent (1.2e-3..6.9e-3) at shipped radial_turnover=5400 s, the accuracy floor under the Robin correction; exact only for wall-parallel flow. |
| **Dynamic mucin carbon liberation is dimensionally wrong** | Units fixed; stoichiometry still open | Resolved half: `k_liberation` is now declared and defaulted as a per-cell specific rate, `5.0e-16 mol/(cell·s)`, so `k_liberation * vbf.density * Monod(M)` is a genuine `mol/m^3/s` and reproduces the static `vbf.mucin_liberation` term it replaces at default density (`5e-16 * 1e11 * 0.909 = 4.5e-5` against `5.0e-5`). No host or device expression changed, so the backends cannot diverge. Derivation in `docs/UNITS_AUDIT.md` §11. Still open: liberation is not limited by the mucin actually present and ignores the `mucin_z_gradient` weighting the static path applies. Original finding: `dynamic_mucin_liberation()` returned `k_liberation [1/s] * vbf.density [cells/m^3] * Monod(M)`, i.e. `1e-4 * 1e11 = 1e7` mol/m^3/s at defaults — eleven orders above the static `vbf.mucin_liberation` term it replaces. Measured with only `mucin.enabled=true`: carbon reaches `3.4e7` mol/m^3 at 26 µm after one 60 s step against a `5e-3` physiological scale, then washes out over roughly ten steps. Mucin is also emptied outside the epithelial layer in one step while carbon receives the full unlimited amount, so the two species are not stoichiometrically coupled, and `mucin.secretion_rate` is inert because liberation is Monod-saturated (`M=1e-2` vs `Km_degradation=1e-3`). `mucin.enabled` is `false` by default, so no shipped result is affected; a mucin-driven carbon arm must not be run until `k_liberation` is redefined as a per-organism rate (mol/(cell·s)) with mucin-limited release. See `docs/CARBON_LADDER_CAMPAIGN.md`. |
...
| **VBF drag made the agent velocity update a permanent no-op** | Fixed | The audit called `drag_force()` "stored but unused"; it was in fact called from `Simulation::module_physics()`, but `a.v` was written *only* by that `-c·v` term, agents start at `v = 0`, and overdamped mechanics never writes `a.v`, so velocity stayed identically zero and `a.x += a.v·dt` never moved anything. Measured over a 60-step VBF-enabled serial run: `max|a.v| = 0` exactly, `max|a.v|·dt = 0`. The explicit update was also unconditionally unstable had `v` ever been nonzero (`c·dt/m = 6.6e7` at `drag_coeff = 1e-9`, `m = 9.1e-16` kg, `dt = 60` s), which rules out entrainment at `bio_dt` without an implicit or analytic relaxation. Removed rather than wired: the velocity integration, the `a.x += a.v·dt` update, `VBF::drag_force()`, the uncalled `VBF::local_capacity()`, and the now-dead `vbf.drag_coeff` / `vbf.carrying_cap` config keys. Agent translation is advection plus overdamped mechanics displacement. `Agent::v` itself is retained (MPI transfer and checkpoint format) and stays zero. |
| **Python manifest writes were non-atomic** | Fixed | `save_manifest()` and the batch runner's run-level JSON writer opened the destination in `"w"`, so an interruption left a truncated or empty manifest with the previous good generation already gone — a real data-loss path on long Spot campaigns. Writes now go through the hardened `path_utils.write_text_file()`, which serializes fully, writes a same-directory `mkstemp` temp file, `flush()` + `os.fsync()`, `os.replace()`, then best-effort directory fsync (directory durability is *not* claimed where the directory fsync is unsupported, and the temp file is removed on any failure). Path validation and the zero-`pythonsecurity:S6549` requirement are unchanged. |
