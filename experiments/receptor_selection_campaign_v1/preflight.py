...
def baseline_is_ancestor(head):
 subprocess.check_call(['git','-C',str(REPO),'cat-file','-e',BASELINE+'^{commit}'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
 return subprocess.call(['git','-C',str(REPO),'merge-base','--is-ancestor',BASELINE,head],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)==0
def verify_baseline_in_checkout(head,context):
 try:
  if not baseline_is_ancestor(head): fail(f'audited model baseline {BASELINE} is not an ancestor of execution checkout HEAD {head}')
 except Exception as e: fail(f'cannot verify model baseline {context}: {e}')
def verify_baseline(head,deployment=False):
 if deployment:
  ...
  verify_baseline_in_checkout(head,'ancestry in deployment checkout'); return
 ...
 if manifest_path.exists():
  ...
  return
 if head: verify_baseline_in_checkout(head,'in git checkout')
...
def check_committed_package():
 ...
 try: dirty=git('status','--porcelain','--',*[str(rel/name) for name in CORE_FILES])
...
def check_execution_identity(a,head):
 if not a.deployment:
  if a.execution_source_sha: fail('--execution-source-sha is only accepted with --deployment')
  return
...
FIXED_SCALARS=[('gpu_enabled',True),('gpu_device_id',0),('total_time',21600),('bio_dt',60),('domain_x',1e-4),('domain_y',1e-4),('domain_z',1e-4),('grid_dx',2e-6)]
HDF5_SCHEDULE=[('summary',1),('agents',10),('provenance',10),('lineage',0),('genome',0)]
def check_run_provenance(rid,cfg,stage,expected_exec):
 meta=cfg.get('_campaign',{})
...
def check_run_hdf5(rid,cfg,stage):
 sch=cfg.get('hdf5',{}).get('schedule',{})
 for k,v in HDF5_SCHEDULE:
...
def check_run_config(stage,rid,cfg,expected_exec):
 check_run_provenance(rid,cfg,stage,expected_exec)
 check_run_scalars(rid,cfg)
 check_run_hdf5(rid,cfg,stage)
 if len(cfg.get('initial_strains',[]))!=2 or [x.get('count') for x in cfg['initial_strains']]!=[60,60]: fail(f'{rid}: founder axis drift')
class Inventory:
 def __init__(self):
  self.configs={}; self.ids=[]; self.signatures=[]; self.image_digests=set(); self.execution_shas=set(); self.baseline_shas=set()
def check_stage_manifest(stage,n,expected_exec,inv):
 mp=G/f'stage_{stage}'/'manifest.json'
 if not mp.exists(): fail(f'missing {mp.relative_to(ROOT)}'); return
 m=json.loads(mp.read_text()); inv.image_digests.add(m.get('container_image_digest')); inv.execution_shas.add(m.get('execution_source_sha')); inv.baseline_shas.add(m.get('model_baseline_sha')); runs=m.get('runs',[])
 ...
 for i,e in enumerate(runs):
  ...
  cfg=json.loads(raw); rid=e['run_id']; inv.ids.append(rid); inv.configs[rid]=cfg
  check_run_config(stage,rid,cfg,expected_exec)
  inv.signatures.append((stage,rid,json.dumps(strip_meta(cfg),sort_keys=True)))
def load_campaign_manifest():
 try: return json.loads((G/'campaign_manifest.json').read_text())
 except Exception as e: fail(f'campaign manifest missing/unreadable: {e}'); return {}
def check_campaign_manifest(campaign_manifest,expected_exec):
 if not campaign_manifest: return
 for k,want in [('schema_version',2),('model_baseline_sha',BASELINE),('execution_source_sha',expected_exec)]:
...
def check_stage_d(configs):
 ...
 check_stage_d_arms(DP,DN); check_stage_d_carrier_lysis(DP); check_stage_d_null_controls(DN)
def check_design(inv):
 if len(inv.ids)!=len(set(inv.ids)): fail('run-id collision')
 if len(inv.signatures)!=65: fail(f'total explicit runs {len(inv.signatures)} != 65')
 q=[s for st,_,s in inv.signatures if st=='Q']
 if len(set(q))!=1: fail('Q repeats are not parser-identical')
 check_stage_a(inv.configs); check_stage_b(inv.configs); check_stage_c(inv.configs); check_stage_d(inv.configs)
def check_provenance_sets(a,inv):
 if len(inv.image_digests)!=1: fail(f'mixed image digests: {inv.image_digests}')
 image=next(iter(inv.image_digests),None); pattern=C['execution']['image_digest_pattern']
 ...
 if a.deployment and inv.execution_shas!={a.execution_source_sha}: fail(f'mixed/wrong execution source SHAs: {inv.execution_shas}')
 if inv.baseline_shas!={BASELINE}: fail(f'mixed/wrong model baseline SHAs: {inv.baseline_shas}')
def main():
 ...
 check_execution_identity(a,head)
 check_decision_record()
 expected_exec=a.execution_source_sha if a.deployment else EXEC_PLACEHOLDER
 campaign_manifest=load_campaign_manifest()
 check_parser_keys()
 inv=Inventory()
 for stage,n in {'Q':2,'A':24,'B':12,'C':12,'D':15}.items(): check_stage_manifest(stage,n,expected_exec,inv)
 check_campaign_manifest(campaign_manifest,expected_exec)
 check_design(inv)
 check_provenance_sets(a,inv)
 print(json.dumps({'status':'FAIL' if ERR else 'PASS','mode':'deployment' if a.deployment else 'planning','model_baseline_sha':BASELINE,'execution_source_sha':next(iter(inv.execution_shas),None),'git_head':head,'runs_checked':len(inv.signatures),'errors':ERR,'warnings':WARN},indent=2))
