...
def validate_scalars(rid: str, cfg: dict, amp: float) -> None:
    exact = {
...
def validate_strains(rid: str, cfg: dict, arm) -> None:
...
def validate_config(entry: dict, cfg: dict, expected_sha: str) -> None:
    ...
    arm = entry.get("arm")
    validate_scalars(rid, cfg, amp)
    validate_strains(rid, cfg, arm)
...
def load_manifest() -> dict:
    try: return json.loads((GENERATED / "manifest.json").read_text())
    except Exception as exc:
        fail(f"generated deployment manifest missing/unreadable: {exc}"); return {}

def validate_manifest_header(manifest: dict, expected_sha: str, deployment: bool) -> None:
...
def resolve_input(rid: str, entry: dict, index: int) -> Path | None:
    """Return the validated input path, or None after recording why it is unusable."""
    relraw = entry.get("input_relpath")
    if not isinstance(relraw, str) or "\\" in relraw or PurePosixPath(relraw).as_posix() != relraw:
        fail(f"{rid}: input_relpath is not portable POSIX syntax: {relraw!r}"); return None
    ...
    except Exception as exc: fail(f"{rid}: input path does not resolve on POSIX: {exc}"); return None
...
def validate_entry(index: int, entry: dict, expected_sha: str, seen: dict) -> None:
    ...
    path = resolve_input(rid, entry, index)
    if path is None: return
    try: cfg = json.loads(path.read_text())
    except Exception as exc: fail(f"{rid}: input JSON unreadable: {exc}"); return
    ...
    if out in seen["outputs"]: fail(f"{rid}: duplicate output path {out!r}")
    seen["outputs"].add(out)
    pair = (float(entry.get("amplitude", -1)), int(entry.get("seed", -1)))
    if entry.get("arm") == "producer": seen["producers"].add(pair)
    elif entry.get("arm") == "toxin_free_null": seen["nulls"].add(pair)
    else: fail(f"{rid}: invalid arm {entry.get('arm')!r}")

def validate_design(seen: dict) -> None:
    wanted_producers = {(float(a), int(seed)) for a in [0,15,20,30,60] for seed in CONTRACT["design"]["seeds"]}
    if seen["producers"] != wanted_producers: fail(f"producer amplitude/seed set mismatch: {sorted(seen['producers'])}")
    wanted_nulls = {(60.0, int(seed)) for seed in CONTRACT["design"]["seeds"]}
    if seen["nulls"] != wanted_nulls: fail(f"null set mismatch: {sorted(seen['nulls'])}")
...
def main() -> int:
    ...
    validate_manifest_header(manifest, expected_sha, args.deployment)
    runs = manifest.get("runs") or []
    if len(runs) != 18: fail(f"manifest has {len(runs)} jobs, expected 18")
    seen = {"producers": set(), "nulls": set(), "outputs": set()}
    for index, entry in enumerate(runs): validate_entry(index, entry, expected_sha, seen)
    validate_design(seen)
