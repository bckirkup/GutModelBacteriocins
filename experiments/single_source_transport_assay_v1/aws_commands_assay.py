...
def validate_head(expected_sha: str) -> str:
    try: head = git("rev-parse", "HEAD")
    except Exception as exc: refuse(f"normal git checkout required: {exc}")
    if head != expected_sha: refuse(f"execution SHA differs from git HEAD {head}")
    return head

def validate_manifest(head: str, image_uri: str) -> None:
    ...
    if manifest.get("container_image_digest") != image_uri: refuse("--image-uri is not byte-for-byte equal to deployment manifest image")
...
def run_deployment_preflight(head: str) -> None:
...
def validate_authorization(path: Path, head: str, image_uri: str) -> None:
    try: auth = json.loads(validate_input_path(path).read_text())
    except (OSError, json.JSONDecodeError, PathValidationError) as exc: refuse(f"authorization file unreadable: {exc}")
    if auth.get("authorize_single_source_assay_submission") is not True or auth.get("execution_source_sha") != head or auth.get("container_image_digest") != image_uri or auth.get("jobs") != 18:
...
def main() -> int:
    ...
    head = validate_head(args.execution_source_sha)
    validate_manifest(head, args.image_uri)
    run_deployment_preflight(head)
    if args.authorization_file: validate_authorization(args.authorization_file, head, args.image_uri)
    q = shlex.quote
