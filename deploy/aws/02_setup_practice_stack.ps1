...
aws batch register-job-definition --job-definition-name $env:JOB_DEFINITION --type container --platform-capabilities EC2 --container-properties "file://$($JDJson.FullName)" --retry-strategy "file://$RetryJson" --timeout "attemptDurationSeconds=$($env:PRACTICE_JOB_TIMEOUT_SECONDS)" --region $env:AWS_REGION | Out-Null
