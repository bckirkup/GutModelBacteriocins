...
| Contraction-driven loss | Partly exists, and this is the most dangerous overlap in the spec: the repository already loses agents through `advection` (peristaltic velocity field), through washout (`washout.trap=emergent` by default, where actual transport determines `outflow_boundary`), and through crypt exit. A Spec 13 contraction event added naively becomes a *fourth* loss channel double-counting the first three. |
