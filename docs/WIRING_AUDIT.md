...
Every rank stores the full chemical grid but contributes reactions from only its local agents. `ChemicalField::sum_reactions_across_ranks` performs an `MPI_Allreduce` before the identical global VBF terms are added, so all ranks diffuse the same global field without double-counting VBF. On GPU-active steps, host-written reactions are uploaded before device reaction kernels and the accumulated device buffer is downloaded only when host reduction or coupling is required; this preserves one authoritative reaction field and synchronizes resulting concentrations back after integration.
