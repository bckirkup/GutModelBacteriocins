...
std::vector<Probe> build_probes() {
  ...
  v.push_back(S("chemistry.toxin_evaluation",
                [](const SimulationConfig& c) { return c.qssa.toxin_evaluation; },
                "agents"));
  v.push_back(S("toxin_lumping",
                [](const SimulationConfig& c) { return c.qssa.toxin_lumping; },
                "lumped"));
  v.push_back(S("chemistry.toxin_lumping",
                [](const SimulationConfig& c) { return c.qssa.toxin_lumping; },
                "lumped"));
  ...
  v.push_back(S("carbon_epithelial_boundary",
                [](const SimulationConfig& c) {
                  return c.carbon_epithelial_boundary;
                }, "flux", false));
  add_ns_real(v, "carbon.epithelial_transfer_coeff",
              "carbon_epithelial_transfer_coeff",
              [](const SimulationConfig& c) {
                return c.carbon_epithelial_transfer_coeff;
              });
  add_ns_real(v, "carbon.epithelial_flux", "carbon_epithelial_flux",
              [](const SimulationConfig& c) {
                return c.carbon_epithelial_flux;
              });
  ...
  v.push_back(R("carbon_maintenance_rate",
                [](const SimulationConfig& c) {
                  return c.fixes.metabolism.carbon_maintenance_rate;
                }));
  v.push_back(R("metabolism.carbon_maintenance_rate",
                [](const SimulationConfig& c) {
                  return c.fixes.metabolism.carbon_maintenance_rate;
                }, 2.0e-5, false));
  ...
  add_ns_int(v, "hdf5.schedule.provenance", "hdf5_schedule_provenance",
             [](const SimulationConfig& c) { return static_cast<long long>(c.hdf5.schedule.provenance); });
  ...
  add_ns_string(v, "oxygen.ros_driver", "oxygen_ros_driver",
                ...
                "funded");
  v.push_back(R("delivery_far_field_radius",
                [](const SimulationConfig& c) {
                  return c.fixes.metabolism.delivery_far_field_radius;
                },
                false));
  v.push_back(R("metabolism_delivery_far_field_radius",
                [](const SimulationConfig& c) {
                  return c.fixes.metabolism.delivery_far_field_radius;
                },
                false));
  ...
  add_ns_real(v, "ferrichrome.initial_conc", "ferrichrome_initial_conc", [](const SimulationConfig& c) { return c.chem_env.ferrichrome.initial_conc; });
  add_ns_real(v, "ferrichrome.boundary_conc", "ferrichrome_boundary_conc", [](const SimulationConfig& c) { return c.chem_env.ferrichrome.boundary_conc; });
