...
SimulationConfig make_compress_cfg(std::string_view path,
  ...
  cfg.hdf5.filename = std::string(path);
  cfg.hdf5.compression = std::string(compression);
...
int main() {
  ...
  Simulation sim_raw;
  sim_raw.init(make_compress_cfg(raw_file, "none"));
  sim_raw.run();

  Simulation sim_gzip;
  sim_gzip.init(make_compress_cfg(gzip_file, "gzip"));
  sim_gzip.run();
